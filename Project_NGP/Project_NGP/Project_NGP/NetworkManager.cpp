...
bool NetworkManager::Initialize()
{
	...
	{
		return error_quit(L"소켓 생성 실패!");
	}

	// 소켓 옵션 설정
	// SO_KEEPALIVE
	BOOL Enable = TRUE;
	int result = setsockopt(m_serversocket, SOL_SOCKET, SO_KEEPALIVE,
		(char*)& Enable, sizeof(Enable));
	if (SOCKET_ERROR == result)
	{
		return error_quit(L"소켓 옵션 SO_KEEPALIVE 설정 실패!");
	}
	...
	optval.l_onoff = 1;
	optval.l_linger = 5;
	result = setsockopt(m_serversocket, SOL_SOCKET, SO_LINGER, 
		(char*)&optval, sizeof(optval));
	if (SOCKET_ERROR == result)
	{
		return error_quit(L"소켓 옵션 SO_LINGER 설정 실패!");
	}

	// SO_REVTIMEO
	int timeOutVal = 3000;
	result = setsockopt(m_serversocket, SOL_SOCKET, SO_RCVTIMEO,
		(char*)&timeOutVal, sizeof(timeOutVal));
	if (SOCKET_ERROR == result)
	{
		return error_quit(L"소켓 옵션 SO_REVTIMEO 설정 실패!");
	}
...
bool NetworkManager::error_quit(const WCHAR* msg)
{
	WCHAR* lpMsgBuf;
	FormatMessage(
	...
	MessageBox(g_hWnd, lpMsgBuf, msg, MB_ICONERROR);
	LocalFree(lpMsgBuf);
	return false;
}
