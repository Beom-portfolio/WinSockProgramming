...
bool MainServer::Initialize()
{
	...
	{
		error_quit("소켓의 수신 버퍼 크기 가져오기 실패");
		return false;
	}
	...
	if (SOCKET_ERROR == result)
	{
		error_quit("소켓 옵션 SO_RCVBUF 설정 실패");
		return false;
	}
	...
	if (0 != ::bind(m_listenSocket, (sockaddr*)&serverAddr,
		sizeof(SOCKADDR_IN)))
	{ 
		error_quit("리슨 소켓 바인드 실패");
		return false;
	}
	// 리슨 소켓 상태 리슨으로 변경
	if (0 != listen(m_listenSocket, SOMAXCONN))
	{ 
		error_quit("리슨 소켓 상태 변경 실패");
		return false;
	}
...
bool MainServer::Running()
{
	...
	// 나머지 한 스레드에서는 서버에서 처리할 Scene을 위한것이다.
